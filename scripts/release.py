...
def main() -> int:
    ...
    print()
    print(f"接下来（手动步骤）：")
    print(f"  1. 推送到 GitHub:")
    print(f"     {C.BOLD}git push && git push --tags{C.END}")
    print(f"  2. 上传 exe 到 GitHub Releases 供用户下载:")
    print(f"     打开 https://github.com/<你的用户名>/TimestampTool/releases/new")
    print(f"     选择 tag v{new_version}，把归档 exe 拖进去，Publish release")
    print()
